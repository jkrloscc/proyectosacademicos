	...
	<div id="main-lists">
		...
		<div class="myLists">
			...
			<div id="addList">
				...
				<form method="post" action="AddListServlet">
					
					<h2>${nameNewList}</h2>

					<input class="create" type="submit" value="Save">
					<input type="hidden" name="nameNewList" value="${nameNewList}">
		...
		<c:forEach var="list" items="${listsList}">
			<div class="list">
				...
				<c:forEach var="card" items="${cardsList}">
					...
					<div class="card">
					<a href="CardServlet?cardName=${card.name}&cardId=${card.id}&listId=${list.id}&listName=${list.name}">
					${card.name} </a>
					</div>
					</c:if>   
				</c:forEach>
